...
function getRectangleString(width, height) {
    let first = String.fromCharCode(9484) + String.fromCharCode(9472).repeat(width - 2) + String.fromCharCode(9488) + '\n';
    let second = (String.fromCharCode(9474) + " ".repeat(width - 2) + String.fromCharCode(9474) + '\n').repeat(height - 2);
    let third = String.fromCharCode(9492) + String.fromCharCode(9472).repeat(width - 2) + String.fromCharCode(9496) + '\n';
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < result.length; i++) {
        if(input.includes(str[i])) {
            result[i] = output[input.indexOf(str[i])];
        }
    }
    return result.join('');
}
